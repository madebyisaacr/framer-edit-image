...
export function App() {
    ...
    const hasChanges = Boolean(flipH || flipV || rotate !== 0)
    const canEdit = Boolean(image && !loading)
    const canSave = Boolean(canEdit && hasChanges)
    ...
    const onSaveClick = () => {
        ...
        if (image) {
            ...
            img.onload = async () => {
                try {
                    const imageResult = await transformImage(img, (rotate * 90) as 0 | 90 | 180 | 270, flipH, flipV)
                    ...
                    console.error(error)
                    framer.notify("Failed to save image", { variant: "error" })
                } finally {
                    setLoading(false)
                }
            }
    ...
    return (
        <main className="flex-col gap-2 w-full max-h-[500px] select-none overflow-hidden px-3 pb-3">
            ...
            <button className="framer-button-primary" disabled={!canSave} onClick={onSaveClick}>
                {loading ? <div className="framer-spinner" /> : "Save"}
            </button>
        </main>
    )
}
...
async function transformImage(
    ...
    const { data: inputData } = ctx.getImageData(0, 0, canvas.width, canvas.height)
    ...
    const transformed = new Uint8Array(newWidth * newHeight * channels)
    ...
    for (let y = 0; y < height; y++) {
        for (let x = 0; x < width; x++) {
            ...
            if (flipHorizontal) rotatedX = newWidth - rotatedX - 1
            if (flipVertical) rotatedY = newHeight - rotatedY - 1

            const dstIndex = (rotatedY * newWidth + rotatedX) * channels
            for (let i = 0; i < channels; i++) {
                transformed[dstIndex + i] = inputData[srcIndex + i]
            }
        }
    }
